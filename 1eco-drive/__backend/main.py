...
df_1.to_csv("train_para.csv", header=["google map_boundary", "map_height(km)", "map_wide(km)", "step_length(m)", "wide_grid_num", "height_grid_num", "total_point", "max_train_step in episode", "pre_train_step", "learning_rate", "Google r", "Google charge num", "Google SOC", "Google time"])
    ...
    for episode in range(train_num):  # num of episode
	    ...
	    # how to choose action 
            ...
            if env.status_dir_check != 'OVER_QUERY_LIMIT':
                ...
                #print("s_list type:", type(s_list))
                #print("s_list structure:", s_list)
                #print("action type:", type(action))
                #print("action value:", action)

                # Then your existing code that's causing the error
                #replay_buffer.append((s_list, action, r, s, d))  # Using a tuple

                #s_list_nested = [[s_list[0], s_list[1]]]  # or [[[s_list[0]], [[s_list[1]]]]] depending on expected structure
                s_list_nested = [[s_list, action]]
                replay_buffer.append((s_list_nested, action, r, s, d))
